...
# 查找 ttyUSB* 设备
def find_ttyUSB():
    print('imu default Serial /dev/ttyUSB0, modify the serial port in the launch file')
    posts = [port.device for port in serial.tools.list_ports.comports() if 'USB' in port.device]
    print('current is connected to {} serial device {} Num {}'.format('USB', len(posts), posts))
    ...
    if key < 11:  # 根据数据长度位的判断, 来获取对应长度数据
        ...
        if buff[1] == 0x51 :
            ...
            if checkSum(data_buff[0:10], data_buff[10]):
                ...
                print('0x5f Verification failed')
        ...
        if angle_flag:
            ...
            imu_msg.orientation.x = angle_degree[0]
            imu_msg.orientation.y = angle_degree[1]
            imu_msg.orientation.z = angle_degree[2]
...
def callback(data):
    ...
    unlock_imu_cmd = '\xff\xaa\x69\x88\xb5'.encode()
    ...
    set_rsw_demo_cmd = '\xff\xaa\x02\x1f\x00'.encode()   #output time acc gyro angle mag
...
if __name__ == "__main__":
    ...
    try:
        ...
        if wt_imu.isOpen():
            rospy.loginfo("\033[32m serial lose1 (ignore this)...\033[0m")
        else:
            wt_imu.open()
            rospy.loginfo("\033[32m serial lose 2...\033[0m")
    except Exception as e:
        print(e)
        rospy.loginfo("\033[31m serial lose\033[0m")
        exit(0)
    # else:
    #     AutoScanSensor()
    ...
    while not rospy.is_shutdown():
        try:
            ...
            if buff_count > 0 and iapflag == 0:
                buff_data = wt_imu.read(buff_count)
                if recordflag:
                    recordbuff = recordbuff + str(buff_data)
